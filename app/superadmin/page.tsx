    ...
    return (
        <div className="p-8 lg:p-10 space-y-10 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <button onClick={fetchStats} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 text-zinc-400 hover:text-amber-500 hover:bg-amber-500/5 transition-all text-xs font-black uppercase tracking-widest">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    ...
                    <div key={i} className="group relative p-6 rounded-[2rem] border transition-all hover:scale-[1.02] hover:border-amber-500/30 overflow-hidden bg-zinc-50 dark:bg-white/[0.02] border-zinc-200 dark:border-zinc-800/50 shadow-sm dark:shadow-none">
                        <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:opacity-[0.06] transition-opacity">
                            <item.icon className="h-24 w-24 text-zinc-900 dark:text-white" />
                        </div>
                        <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-600 uppercase tracking-[0.2em] mb-3">{item.label}</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* User Distribution */}
                <div className="lg:col-span-2 rounded-[2.5rem] border p-8 space-y-8 bg-zinc-50/50 dark:bg-white/[0.01] border-zinc-200 dark:border-zinc-800/50 shadow-sm dark:shadow-none">
                    <div className="flex items-center justify-between">
                        ...
                        <TrendingUp className="h-4 w-4 text-zinc-300 dark:text-zinc-700" />
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-600 uppercase tracking-widest">Student Population</p>
                            ...
                            <div className="h-2 w-full bg-zinc-200 dark:bg-zinc-900 rounded-full overflow-hidden flex">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-600 uppercase tracking-widest">Administrative Force</p>
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-sm font-black text-zinc-500 dark:text-zinc-400">{getUserStat('superadmin').total}</p>
                                </div>
                            </div>
                            ...
                            <p className="text-[9px] font-bold text-zinc-400 dark:text-zinc-600 uppercase tracking-wider italic">
                ...
                <div className="rounded-[2.5rem] border p-8 space-y-6 bg-zinc-50/50 dark:bg-white/[0.01] border-zinc-200 dark:border-zinc-800/50 shadow-sm dark:shadow-none">
                    <div className="flex items-center justify-between">
                        <h4 className="text-lg font-black text-zinc-900 dark:text-white flex items-center gap-2">
                            <Clock className="h-5 w-5 text-zinc-400 dark:text-zinc-500" />
                            Activity Feed
                        </h4>
                        <div className="px-2 py-1 rounded-md bg-zinc-200 dark:bg-zinc-900 text-[8px] font-black text-zinc-500 uppercase tracking-widest">
                            Live
                        </div>
                    </div>

                    <div className="space-y-6 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[1px] before:bg-zinc-200 dark:before:bg-zinc-800">
                        {stats?.recentEvents?.map((event: any, i: number) => (
                            <div key={i} className="relative pl-8 animate-in slide-in-from-left duration-300" style={{ animationDelay: `${i * 100}ms` }}>
                                <div className="absolute left-0 top-1.5 h-6 w-6 rounded-lg bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center z-10 shadow-sm">
                                    <div className="h-1.5 w-1.5 rounded-full bg-amber-500 shadow-[0_0_5px_rgba(245,158,11,0.8)]" />
                                </div>
                                <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-0.5">{event.actor_name}</p>
                                <p className="text-xs font-bold text-zinc-700 dark:text-zinc-200 line-clamp-1">{event.action}</p>
                                <p className="text-[9px] font-medium text-zinc-400 dark:text-zinc-600 mt-1">{new Date(event.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full py-3 rounded-2xl bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest hover:text-amber-500 dark:hover:text-white hover:border-amber-500/20 transition-all shadow-sm dark:shadow-none">
                        View Complete Logs
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Maintenance Monitor */}
                <div className="p-8 rounded-[2rem] border overflow-hidden relative bg-zinc-50/50 dark:bg-white/[0.01] border-zinc-200 dark:border-zinc-800/50 shadow-sm dark:shadow-none">
                    ...
                    <div className="space-y-4">
                        {stats?.complaints?.map((c: any, i: number) => (
                            <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-white dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/50 shadow-sm dark:shadow-none">
                                <div className="flex items-center gap-3">
                                    <div className={cn("h-2 w-2 rounded-full", c.status === 'Pending' ? 'bg-rose-500' : 'bg-amber-500')} />
                                    <span className="text-xs font-bold text-zinc-600 dark:text-zinc-300">{c.status} Complaints</span>
                                </div>
                ...
                <div className="p-8 rounded-[2rem] border relative overflow-hidden flex flex-col justify-center gap-4 border-zinc-200 dark:border-zinc-800/50 bg-zinc-50 dark:bg-white/[0.01] shadow-sm dark:shadow-none"
                    ...
                    <div className="relative">
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            <button onClick={() => window.location.href = '/admin'} className="px-6 py-2.5 rounded-xl border border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 text-[10px] font-black uppercase tracking-widest hover:border-amber-500 transition-all bg-white dark:bg-transparent">
                                Admin Dashboard
                            </button>
                        </div>
                    </div>
